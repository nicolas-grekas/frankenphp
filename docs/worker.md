...
The wake-up sent at start makes the callback run as soon as the loop does, which is when the worker becomes ready. The polling API of PHP 8.6 works the same way: wrap the stream in a `StreamPollHandle`, add it to a context, and tick when it triggers.
...
Sixteen updates are buffered per task; past that, `update()` waits for the sender to read, and it throws once the sender gave up. The streams of a task are backed by eventfd descriptors on Linux, pooled between tasks, and by a socket pair elsewhere.
