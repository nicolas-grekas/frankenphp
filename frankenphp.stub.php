 ...
 * park on stream_select() and exit its loop gracefully. Every call of a run
 * returns the same stream, a fresh one over the same socket once the script
 * closed it. The worker counts as ready, and its startup as successful, once
 * it waits on the stream (stream_select() or a blocking read). Only callable
 * from inside a background worker.
 *
 * @return resource
 */
 ...
 * frankenphp_get_worker_handle() carries a line when a task waits for the
 * thread, so a script polls after every read of it. A line is a wake-up
 * rather than a description: its content is unspecified and must not be
 * inspected, it is not a count either, and null after one is expected in a
 * pool. $stream reaches EOF when the sender closes its own stream, for
 * stream_select() and feof(). Only callable from inside a background
 * worker.
 *
