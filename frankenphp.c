 ...
 * sent to the worker, see frankenphp_send_task(). The line is a wake-up
 ...
 * contract, see frankenphp_receive_task(). */
...
static ssize_t frankenphp_worker_handle_read(php_stream *stream, char *buf,
  ...
  uintptr_t idx = frankenphp_thread_index();
  if (count >= 1 && go_frankenphp_background_worker_wait(idx, false)) {
    buf[0] = '\n';
