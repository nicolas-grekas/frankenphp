...
/* Handles of a task: thin objects over the per-task stream of each side.
 * The stream carries the task, the object holds its resource, so
 * getStream() hands it to stream_select() and dropping the last reference
 * ends the task, as closing the stream always did. */
typedef struct {
  zend_resource *res;
...
static zend_object_handlers frankenphp_task_obj_handlers;

static frankenphp_task_obj *frankenphp_task_obj_of(zend_object *object) {
  return (frankenphp_task_obj *)((char *)object -
                                 XtOffsetOf(frankenphp_task_obj, std));
}

static zend_object *frankenphp_task_obj_create(zend_class_entry *ce) {
  frankenphp_task_obj *obj = zend_object_alloc(sizeof(*obj), ce);

  zend_object_std_init(&obj->std, ce);
  object_properties_init(&obj->std, ce);
  obj->std.handlers = &frankenphp_task_obj_handlers;
  obj->res = NULL;
...
static void frankenphp_task_obj_free(zend_object *object) {
  frankenphp_task_obj *obj = frankenphp_task_obj_of(object);

  if (obj->res != NULL) {
    /* the last reference closes the stream, which settles the task */
    zend_list_delete(obj->res);
    obj->res = NULL;
  }
...
static void frankenphp_task_obj_take(zval *zthis, php_stream *stream) {
  frankenphp_task_obj_of(Z_OBJ_P(zthis))->res = stream->res;
}
...
static php_stream *frankenphp_task_obj_stream(zval *zthis) {
  frankenphp_task_obj *obj = frankenphp_task_obj_of(Z_OBJ_P(zthis));

  if (obj->res == NULL || obj->res->ptr == NULL ||
      obj->res->type != php_file_le_stream()) {
    return NULL;
  }
...
static void frankenphp_task_obj_get_stream(zval *zthis, zval *return_value) {
  if (frankenphp_task_obj_open_stream(zthis) == NULL) {
    RETURN_THROWS();
  }

  zend_resource *res = frankenphp_task_obj_of(Z_OBJ_P(zthis))->res;
  GC_ADDREF(res);
  RETURN_RES(res);
}
...
static void frankenphp_task_obj_close(zval *zthis) {
  php_stream *stream = frankenphp_task_obj_stream(zthis);
  if (stream != NULL) {
    zend_list_close(frankenphp_task_obj_of(Z_OBJ_P(zthis))->res);
  }
}
...
ZEND_METHOD(FrankenPHP_ReceivedTaskHandle, getPayload) {
  ...
  frankenphp_task_obj *obj = frankenphp_task_obj_of(Z_OBJ_P(ZEND_THIS));
...
ZEND_METHOD(FrankenPHP_WorkerHandle, receive) {
  ...
  frankenphp_task_obj *obj = frankenphp_task_obj_of(Z_OBJ_P(return_value));
  obj->res = stream->res;
  ZVAL_COPY_VALUE(&obj->payload, &payload);
}
...
PHP_MINIT_FUNCTION(frankenphp) {
  ...
  register_class_FrankenPHP_WorkerHandle();

  frankenphp_task_obj_handlers = std_object_handlers;
  frankenphp_task_obj_handlers.offset = XtOffsetOf(frankenphp_task_obj, std);
  frankenphp_task_obj_handlers.free_obj = frankenphp_task_obj_free;
  frankenphp_task_obj_handlers.clone_obj = NULL;
  frankenphp_sent_task_ce = register_class_FrankenPHP_SentTaskHandle();
  frankenphp_sent_task_ce->create_object = frankenphp_task_obj_create;
  frankenphp_received_task_ce = register_class_FrankenPHP_ReceivedTaskHandle();
  frankenphp_received_task_ce->create_object = frankenphp_task_obj_create;
#ifndef PHP_WIN32
