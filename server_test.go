...
func TestServer(t *testing.T) {
	...
	t.Run("name", func(t *testing.T) {
		...
		initServers(t, frankenphp.WithServer(named), frankenphp.WithServer(unnamed), frankenphp.WithServer(alsoNamed))
		...
		// names qualify worker names in metrics, so they are made unique
		assert.Equal(t, "api_1", alsoNamed.Name())
	})
